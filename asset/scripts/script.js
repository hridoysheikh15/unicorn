    ...
    if ((j = 5)) {
      ...
      for (let z = 0; z < 5; z++) {
        ...
        if (z !== j) {
          ...
          document.getElementById("card_content" + (i + n)).style.display =
            "block";
          document
            .getElementsByClassName("dot")
            [i + n].classList.add("active_dot");
...
// all work btn single page design

function getContent(n) {
  for (let i = 0; i < 6; i++) {
  let classStyle = document.querySelectorAll(`.work-btn-list .work-list`);
  if (i !== n) {
      classStyle[i].classList.remove("bg_color");
      document.getElementById("port_list" + i).style.display = "none";
    } else {
      classStyle[i].classList.add("bg_color");
      document.getElementById("port_list" + i).style.display = "block";
    }
